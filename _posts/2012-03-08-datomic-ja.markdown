...
これはつまり数字のインクリメントやCASなどをユーザ自身で定義でき、それがアトミックに処理できるということを意味しています。<br>
...
また、クエリーをローカルで実行するという特性を利用して、なんとアプリ側の言語で書いた関数をクエリー中にフィルターとして含めることができます。すごい。
